...
def _run_file_mode(input_path: str, output_dir: str, params: dict,
    ...
    txt_path = os.path.join(output_dir,
                            f'trapezoid_file_{time.strftime("%Y_%m_%d_%H_%M_%S")}.txt')
    ...
    log(f"总计: {total_tests}, PASS: {pass_count}, FAIL: {fail_count}", "INFO")
    ...
    return {
        ...
        'message': f"文件模式测试完成: {total_tests} 点, PASS={pass_count}, FAIL={fail_count}\n结果: {txt_path}",
...
def run(input_path: str, output_dir: str, params: dict,
    ...
    # ---- 文件模式 ----
    if is_file_mode:
        return _run_file_mode(input_path, output_dir, params, log, prog)
    ...
    os.makedirs(output_dir, exist_ok=True)
    txt_path = os.path.join(output_dir,
                            f'trapezoid_scan_{corner_info["name"]}_{time.strftime("%Y_%m_%d_%H_%M_%S")}.txt')
