...
if __name__ == "__main__":
    # 参数设置
    point_name = '左上'  # 可选：'左上'、'右上'、'左下'、'右下'
    show_animation = False  # 设为True显示动画，False显示静态图
    ...
    # 加载对应点的数据
    filepath = os.path.join(script_dir, f'{point_name}.csv')
    ...
    if all_points is not None:
        ...
        # 保存高分辨率图像
        if not show_animation:
            output_path = os.path.join(script_dir, f'{point_name}_trajectory_with_abnormal_{IMAGE_WIDTH}x{IMAGE_HEIGHT}.png')
